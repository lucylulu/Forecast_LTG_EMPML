...
results_mean = as.data.frame(colMeans(dataplot)[-1])
...
for(m in seq(length(end_dates))){   
  ...
  ########predict analyst error  ML+man only 
  clf = h2o.randomForest(x= inputvarman2,y='error_f2',training_frame=training_all2,validation_frame = validation_all)
...
results_mean = cbind(results_mean,colMeans(dataplot)[-1])   

colnames(results_mean) = c('1year forecast','2year forecast')

write.csv(results_mean,'results_mean.csv')
